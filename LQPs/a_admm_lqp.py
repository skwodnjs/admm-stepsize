...
def admm_LQP(param, theta=1, tol=1e-8, max_iter=1000):
    start = time.time()
...
if __name__ == "__main__":
    # seed = 42 # 42827 에서 theta 구하는데 거의 10초 걸림
    seed = np.random.randint(0, 100000)
    np.random.seed(seed)
    print(f"seed: {seed}")
    param = generate_random_param_of_LQP(m=3, n=5)

    _, obj1, iter_count, admm_time = admm_LQP(param)
    theta_opt, opt_theta_time = optimal_theta(param)
    _, obj2, iter_count_opt, admm_time_opt = admm_LQP(param, theta=theta_opt)
